    ...
    with open(fsql, mode='w') as sql_file:
      with open(f, mode='r') as csv_file:
          ...
          for row in csv_reader:
              ...
              if city_name_before != row['kabupaten']:
                  city_name_before = row['kabupaten']
                  city_id += 1
                  if "'" in row["kabupaten"]:
                  ...
                  print(f'Kota {city_name_before} {province_id}')
              
              if district_name_before != row["kecamatan"]:
                district_name_before = row["kecamatan"]
                district_id += 1
                if "'" in row["kecamatan"]:
                  row["kecamatan"] = row["kecamatan"].replace("'", "''")
                sql_file.write('INSERT INTO districts(id, city_id, name) VALUES({}, {}, "{}");\n'.format(district_id, city_id, row["kecamatan"]))
              
              subdistrict_id += 1
              if "'" in row["kelurahan"]:
                  row["kelurahan"] = row["kelurahan"].replace("'", "''")
              sql_file.write('INSERT INTO subdistricts(id, district_id, name, zip_code) VALUES({}, {}, "{}", "{}");\n'.format(subdistrict_id, district_id, row["kelurahan"], row["kodepos"]))
              line_count += 1
